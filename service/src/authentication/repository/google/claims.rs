...
impl From<ClaimsError> for CompleteAuthError {
  fn from(e: ClaimsError) -> Self {
    match e {
      ClaimsError::MissingIdToken => {
        CompleteAuthError::AuthenticationError("ID Token was not present in response".to_owned())
      }
      ClaimsError::DecodeError(_) => {
        CompleteAuthError::AuthenticationError("Receive malformed ID Token from Google".to_owned())
      }
    }
  }
}
